		...
		if ($_POST) {
			...
			if (!$uemail || !$upass) {
				...
				if ($login) {
					...
					if ($remember == 1)
						$this -> cache -> memcached -> save('__login', array('uid' => $login[0] -> uid, 'uemail' => $uemail, 'ufaculty' => $login[0] -> ufaculty, 'ugid' => $login[0] -> ugid, 'permission' => $permission, 'ldate' => time(), 'lip' => ip2long($_SERVER['REMOTE_ADDR']), 'skey' => md5(sha1($login[0] -> ugid.$uemail) . 'dist')), time()+60*60*24*100);
					else
						$this -> cache -> memcached -> save('__login', array('uid' => $login[0] -> uid, 'uemail' => $uemail, 'ufaculty' => $login[0] -> ufaculty, 'ugid' => $login[0] -> ugid, 'permission' => $permission, 'ldate' => time(), 'lip' => ip2long($_SERVER['REMOTE_ADDR']), 'skey' => md5(sha1($login[0] -> ugid.$uemail) . 'dist')), 3600);
