$(function() {
    ...
    $(window).ready(i), $(window).on("resize", i), $(".sidebartoggler").on("click", function() {
            ...
            if (input.files) {
                ...
                for (i = 0; i < filesAmount; i++) {
                    ...
                    reader.onload = function(event) {
                        $($.parseHTML('<img style="width:400px">')).attr('src', event.target.result).appendTo(placeToInsertImagePreview);
                    }
        ...
        $(document).ready(function() {
            $('#slideshow-picture-add').on('change', function() {
                $('.current-image-slideshow').remove()
                $('.slideshow-preview > img').remove()
                imagesPreview(this, 'div.slideshow-preview');
            });
            ...
            $('select[name="switchfaculty"]').change(function(){
                window.location.href = '/switchfaculty/' + $(this).val();
            });

            $('.card-body > .basic-form > form input').attr('autocomplete', 'off')

            tinymce.init({
              selector: '.textarea_editor',
              height: 500,
              theme: 'modern',
              plugins: 'print preview searchreplace autolink paste directionality visualblocks visualchars fullscreen image link media template code codesample table charmap hr pagebreak nonbreaking anchor toc insertdatetime advlist lists textcolor wordcount imagetools  contextmenu colorpicker textpattern help',
              toolbar1: 'formatselect | bold italic strikethrough forecolor backcolor | image media | link | alignleft aligncenter alignright alignjustify  | numlist bullist outdent indent  | removeformat',
              image_advtab: true,
              relative_urls : false,
